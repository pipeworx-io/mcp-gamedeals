...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Gamedeals');
}

const BASE_URL = 'https://www.cheapshark.com/api/1.0';

// CheapShark started rejecting requests with no User-Agent ("Missing or
// generic User-Agent header detected") around 2024. CF Workers' fetch
// sends no UA by default, so every call from the gateway 400'd until
// this header was added. Caught by Chiark agent quality benchmark
// (100% errors on list_stores in production).
const FETCH_HEADERS = { 'User-Agent': 'Pipeworx-Gateway/1.0 (https://pipeworx.io)' };
...
async function searchDeals(args: SearchDealsArgs) {
  ...
  const res = await pwFetch(`${BASE_URL}/deals?${params.toString()}`, { headers: FETCH_HEADERS });
  if (!res.ok) throw await httpError(res, 'CheapShark error');

  const data = (await parseJson<unknown>(res, 'CheapShark')) as RawDeal[];
...
async function searchGames(query: string, limit: number) {
  ...
  const res = await pwFetch(`${BASE_URL}/games?${params.toString()}`, { headers: FETCH_HEADERS });
  if (!res.ok) throw await httpError(res, 'CheapShark error');

  const data = (await parseJson<unknown>(res, 'CheapShark')) as RawGame[];
...
async function getGameDetails(id: string) {
  const res = await pwFetch(`${BASE_URL}/games?id=${encodeURIComponent(id)}`, { headers: FETCH_HEADERS });
  if (!res.ok) throw await httpError(res, 'CheapShark error');

  const data = (await parseJson<unknown>(res, 'CheapShark')) as RawGameDetails;
...
async function listStores() {
  const res = await pwFetch(`${BASE_URL}/stores`, { headers: FETCH_HEADERS });
  if (!res.ok) throw await httpError(res, 'CheapShark error');

  const data = (await parseJson<unknown>(res, 'CheapShark')) as RawStore[];
